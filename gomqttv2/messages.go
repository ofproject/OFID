...
func (h *Header) encodeInto(buf *bytes.Buffer, msgType MessageType, remainingLength int32) error {
	...
	val := byte(msgType) << 4
	val |= (boolToByte(h.DupFlag) << 3)
	val |= byte(h.QosLevel) << 1
	val |= boolToByte(h.Retain)
	buf.WriteByte(val)
...
func encodeAckCommon(w io.Writer, header *Header, messageId uint16, msgType MessageType) error {
	buf := new(bytes.Buffer)
